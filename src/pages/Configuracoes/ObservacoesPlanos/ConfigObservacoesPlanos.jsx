...
import {
    excluirPlanosObservacao,
    listarPlanosObservacoes,
    listarProcedimentosParaObservacoes,
    normalizarCodigoProcedimento,
    salvarPlanosObservacao,
} from '../../../lib/impressaoPlanos/planosObservacoes.js'
...
import './ConfigObservacoesPlanos.css'
...
function ChipsProcedimentos({
    ...
    return (
        <div className="config_obs_planos_chips_wrap">
            <div className="config_obs_planos_chips">
                {visiveis.map((codigo) =>
                    removivel ? (
                        <button
                            key={codigo}
                            type="button"
                            className="config_obs_planos_chip"
                            disabled={disabled}
                        ...
                        <span key={codigo} className="config_obs_planos_chip is-static">
                            {rotuloProc(codigo)}
                        </span>
                    ),
                )}
            ...
            {ocultos ? (
                <button
                    type="button"
                    className="config_obs_planos_chips_toggle"
...
const ConfigObservacoesPlanos = () => {
    const profile = getStoredAccessProfile()
    const somenteLeitura = useMemo(() => {
        if (getReadOnlyFlag()) return true
        if (!profile) return true
        if (usuarioPodeEditarFerramenta(profile.permissions, 'configuracoes.observacoes_planos')) {
            return false
        }
    ...
    const salvar = async () => {
        ...
        try {
            ...
            const res = await salvarPlanosObservacao({
    ...
    const excluir = async (item) => {
        ...
        setFeedback('')
        try {
            const res = await excluirPlanosObservacao(item.id)
            if (!res.ok) {
    ...
    return (
        <div
            className={`el-page credenciamento_main config_obs_planos${
                formAberto ? ' is-editing' : ''
            }`}
        >
            <PageHeader
                kicker="Configurações"
                title="Observações (Planos)"
                description="Mensagens no modelo do PDF de planos (N) Título: texto). Só entram se os procedimentos vinculados estiverem no documento. Sem gatilhos, usam-se as páginas do PDF molde."
            />
            ...
            {!formAberto ? (
                <div className="config_obs_planos_toolbar">
                    <CampoBuscaComLimpar
                        className="credenciamento_main_input config_obs_planos_busca"
                        value={buscaObs}
                        onChange={(e) => setBuscaObs(e.target.value)}
                        placeholder="Buscar observação ou procedimento…"
                    />
                    <div className="config_obs_planos_toolbar_acoes">
                        {!somenteLeitura ? (
                            <button
                                type="button"
                            ...
                            <span className="config_obs_planos_muted">Somente leitura</span>
                        )}
                    </div>
                </div>
            ) : null}
            ...
            {formAberto ? (
                <section
                    ref={formRef}
                    className="config_obs_planos_form"
                    aria-label="Formulário de observação"
                >
                    <div className="config_obs_planos_form_head">
                    ...
                    <label className="config_obs_planos_campo">
                    ...
                    <label className="config_obs_planos_campo">
                        <span>Texto</span>
                        <textarea
                            className="credenciamento_main_input config_obs_planos_textarea"
                            rows={5}
                            value={form.mensagem}
                            disabled={somenteLeitura || salvando}
                            onChange={(e) =>
                    ...
                    <p className="config_obs_planos_preview_label">Prévia no PDF</p>
                    <div className="config_obs_planos_preview" aria-hidden>
                        <p>
                            <strong>
                    ...
                    <div className="config_obs_planos_form_meta">
                        <label className="config_obs_planos_campo config_obs_planos_campo_curto">
                            <span>Ordem</span>
                            <input
                                type="number"
                                className="credenciamento_main_input"
                        ...
                        <label className="config_obs_planos_check_ativa">
                            <input
                                type="checkbox"
                                checked={form.ativa}
                                disabled={somenteLeitura || salvando}
                    ...
                    <div className="config_obs_planos_procs">
                        <div className="config_obs_planos_procs_head">
                        ...
                        {form.codigosSelecionados.length ? (
                            ...
                            <p className="config_obs_planos_muted">
                        ...
                        <div className="config_obs_planos_procs_lista">
                            {categoriasProc.map((cat) => {
                                ...
                                return (
                                    <section key={cat.id} className="config_obs_planos_cat">
                                        <div className="config_obs_planos_cat_head">
                                            <h4>
                                                {cat.nome}
                                                <em>
                                                    {' '}
                                            ...
                                            <div className="config_obs_planos_cat_acoes">
                                                <button
                                                    type="button"
                                                    className="config_obs_planos_link"
                                                    disabled={somenteLeitura || salvando}
                                        ...
                                        {procsVisiveis.map((p) => {
                                            ...
                                            return (
                                                <label
                                                    key={p.codigo}
                                                    className="config_obs_planos_proc_item"
                                                >
                                                    <input
                                                        type="checkbox"
                                                        checked={marcado}
                    ...
                    {!somenteLeitura ? (
                        <div className="config_obs_planos_form_acoes">
                            <button
                                type="button"
                                className="credenciamento_main_action_btn secondary"
                                disabled={salvando}
            ...
            {!formAberto && loading ? (
                <p className="config_obs_planos_muted">A carregar…</p>
            ) : null}

            {!formAberto && !loading ? (
                <div className="config_obs_planos_lista">
                    {observacoesFiltradas.map((item) => (
                        <article
                            key={item.id}
                            className={`config_obs_planos_card ${item.ativa ? '' : 'is-inativa'}`}
                        >
                            <div className="config_obs_planos_card_top">
                                <span className="config_obs_planos_card_ordem">#{item.ordem}</span>
                                {!item.ativa ? (
                                    <span className="config_obs_planos_badge">Inativa</span>
                                ) : (
                                    <span className="config_obs_planos_badge is-ativa">Ativa</span>
                                )}
                            </div>
                            <p className="config_obs_planos_card_msg">
                                <strong>
                            ...
                            />
                            {!somenteLeitura ? (
                                <div className="config_obs_planos_card_acoes">
                                    <button
                                        type="button"
                                        className="credenciamento_main_action_btn"
                                        disabled={salvando}
...
export default ConfigObservacoesPlanos
