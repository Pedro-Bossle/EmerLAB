...
const SIDEBAR_ICONS = {
  ...
  administrativo: (
    ...
    >
      <path
        d="M12 3.5 19 7v5.2c0 4.1-2.8 7.4-7 8.3-4.2-.9-7-4.2-7-8.3V7l7-3.5Z"
        stroke="currentColor"
        strokeWidth="1.75"
        strokeLinejoin="round"
      />
  ...
  const menuItemsVisiveis = useMemo(
    () =>
      SIDEBAR_GROUPS.map((item) => {
        if (item.permission && !hasPermission(accessProfile, item.permission))
          return null;
        if (item.href && !item.children) {
          if (!podeVerItemMenu(item)) return null;
          return item;
        }
        const children = filtrarChildrenNav(
          item.children || [],
          podeVerItemMenu,
        );
    ...
    >
              ...
              {item.href && !item.children ? (
                ...
                <>
                  <button
                    ...
                    onClick={() =>
                      open ? toggleMenu(item.id) : onToggleManual?.()
                    }
                    title={item.label}
                  >
                    <span className="inline-flex h-5 w-5 shrink-0 items-center justify-center">
      ...
      <div className="flex shrink-0 flex-col gap-1 border-t border-white/10 p-1.5">
        <button
          ...
          title={
            isPinned
              ? "Recolher menu (salvar preferência)"
              : "Manter menu aberto (salvar preferência)"
          }
        >
          ...
          <span className={labelClass}>
            {isPinned ? "Recolher menu" : "Manter aberto"}
          </span>
        </button>
