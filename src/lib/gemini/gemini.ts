/**
 * Cliente Gemini global (servidor). Config só via ENV: GEMINI_*, PROSPECTOS_GEMINI_MAX.
...
function timeoutEnvMs() {
    return envInt('GEMINI_TIMEOUT_MS', 90_000)
}
...
export function prospectosGeminiMax() {
    return envInt('PROSPECTOS_GEMINI_MAX', 20)
}

/** Limites do AI Studio (preencher GEMINI_RPM / GEMINI_RPD no .env). */
export function geminiRpmLimite() {
    return envInt('GEMINI_RPM', 15)
}

export function geminiRpdLimite() {
    return envInt('GEMINI_RPD', 1000)
}
...
export function marcarRateEsgotado(retryAfterSec) {
    const sec = Number(retryAfterSec)
    const espera = Number.isFinite(sec) && sec > 0 ? Math.min(sec, 24 * 3600) : 60
    bloqueadoAteMs = Date.now() + espera * 1000
}

export function lerRate() {
    const agora = Date.now()
    podarHits(agora)
    const rpmLimite = geminiRpmLimite()
    const rpdLimite = geminiRpdLimite()
    ...
    return {
        ok: true,
        modelo: modeloGeminiPadrao(),
        configurado: Boolean(String(process.env.GEMINI_API_KEY || '').trim()),
        rpmUsados,
        rpmLimite,
...
async function geminiGenerateJsonUmaTentativa(apiKey: string, model: string, opts: GeminiGenerateOpts) {
    ...
    try {
        ...
        logGemini('warn', 'falha-json', { model, ms: Date.now() - t0, status: status || null, erro: message })
        return {
            ok: false,
            status,
            erro: message,
    ...
    logGemini('info', 'resposta-json', { model, ms: Date.now() - t0, ...resumo })
    if (!texto) {
        return { ok: false, erro: 'Gemini não retornou conteúdo.', modeloUsado: model }
    }

    try {
        return { ok: true, data: JSON.parse(texto), modeloUsado: model, finishReason: resumo.finishReason }
    } catch {
...
async function executarGenerateComCandidatos(opts, umaTentativa) {
    ...
    if (quotaExceeded) marcarRateEsgotado(retryAfterSec)
...
export const configSnapshot = geminiConfigSnapshot
